...
If we make the field private by changing `pub f32` to `f32`, the struct's implementation details are hidden. This is called _encapsulation_: Within this module, we still have to take special care that the invariant is preserved, but if the code is correct, the public API is impossible to use incorrectly.
